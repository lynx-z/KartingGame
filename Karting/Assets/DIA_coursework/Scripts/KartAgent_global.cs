...
namespace KartGame.AI
{
    ...
    /// </summary>
    public class KartAgent_global : Agent, IInput
    {
...
///global observe
        public void globalObserve()
        {
            int blockInterval = 7;

            int targetIndex = Random.Range(0, otherKarts.Length);
            Transform otherKart = otherKarts[targetIndex];


            float dist = Vector3.Distance(otherKart.position, this.transform.position);
            Vector3 relativePosition = this.transform.InverseTransformPoint(otherKart.position);

            // Vector3 kartDirecion = this.transform.position - otherKart.position;
            // print(kartDirecion);

            if (relativePosition.z<0 && dist <= 10 && findKart < blockInterval ){
                findKart +=1;


            } else if (relativePosition.z<0 && dist <= 10 && findKart >= blockInterval ){
                findKart = 0;

                if ( relativePosition.x > 0.1) block_direction =1;
                else if (relativePosition.x < -0.1)block_direction =-1;
                else block_direction =0;
                // block_direction = relativePosition.x;
                doBlock =true;
