...
////////back Senor;
        public Transform BackSensorTransform;
        public Sensor[] backSensors;  
////////
...
///// show reward on episode
            // float curr_reward = GetCumulativeReward();
            // if (curr_reward > best_reward) best_reward = curr_reward;
            if (step == 1000) print(this.name+"   steps: " + step +"   reward: "+GetCumulativeReward());
            // print(true);
...
/////// 
        ...
        public override void CollectObservations(VectorSensor sensor)
        {
            ...
            // if (ShowRaycasts) Debug.DrawLine(AgentSensorTransform.position, nextCollider.transform.position, Color.magenta);
...
/// positive: right; negative: left
            ...
            if (doBlock == true){
                ...
                // m_Steering = actions[0] - 1f + block_direction;


                // make sure not turn around
                doBlock = false;
...
/// get back sensor info
        public void BackSensorObservations()
        {
            // If a collision is detected in ten consecutive frames, a Block is performed
            int blockInterval = 10;

            // loop all back sensors
            for (var i = 0; i < backSensors.Length; i++)
            {
                var current = backSensors[i];
                var hit = Physics.Raycast(BackSensorTransform.position, current.Transform.forward, out var hitInfo,
                    current.RayDistance, KartMask);

                if (ShowRaycasts){
                    Debug.DrawRay(BackSensorTransform.position, current.Transform.forward * current.RayDistance, Color.blue);
                }

                // if detect other kart
                if (hit && findKart < blockInterval ){
                    findKart +=1;

                }else if(hit && findKart >= blockInterval ){
                    findKart = 0; // reset findKart
                    // set Kart next direction toward other kart
                    block_direction = (float)Math.Round(current.Transform.forward.x); 
                    doBlock =true;
                }        
            }
        }
